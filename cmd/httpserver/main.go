...
import (
	...
	"github.com/lealre/http1-server/internal/request"
	"github.com/lealre/http1-server/internal/response"
	"github.com/lealre/http1-server/internal/server"
)
...
func handler(w *response.Writer, req *request.Request) {
	...
	if req.RequestLine.RequestTarget == "/error" {
		handler500(w, req)
		return
	}
...
<p>Internal Server Error message</p>
</body>
</html>
`)
	h := response.GetDefaultHeaders(len(body))
...
func handlerGetVideo(w *response.Writer, req *request.Request) {
	w.WriteStatusLine(response.Ok)
	body, err := os.ReadFile("assets/sample.mp4")
	if err != nil {
		fmt.Printf("Error reading from assets/sample.mp4")
		handler500(w, req)
		return
	}
	h := response.GetDefaultHeaders(len(body))
